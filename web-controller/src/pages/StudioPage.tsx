...
/** Fallback when a step omits speed (move blocks normally include it). */
...
export function StudioPage() {
  ...
  const [, setLastCommand] = useState("None");
  ...
  async function runBlockProgram(steps: Step[]) {
    ...
    let programDefaultSpeed = DEFAULT_MOVE_SPEED;
    for (const step of steps) {
      ...
      if (step.type === "set_speed") {
        programDefaultSpeed = Math.max(0, Math.min(255, step.value ?? programDefaultSpeed));
        log(`Program default speed set to ${programDefaultSpeed}`);
        continue;
      }
      const command = (step.command ?? "S") as RobotCommand;
      const commandSpeed = step.speed ?? programDefaultSpeed;
    ...
    <main className="app">
      ...
      <div className="workspace-layout">
        ...
        <aside className="side-stack">
          {mode === "simulator" && (
            <SimulatorStage enabled={isDrivingEnabled} heading={simHeading} position={simPosition} path={simPath} />
          )}
